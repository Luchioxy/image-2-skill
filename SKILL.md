...
name: 文生图生成
description: >
  图片生成服务技能。当其他 skill 或 agent 需要生成图片时，调用本技能即可。
  使用 OpenAI DALL·E 模型，支持 DALL·E 2 和 DALL·E 3。
  触发场景：生成图片、创建图片、制作图片、文生图、AI 绘画、图片生成等。
  本技能可被其他 skill 通过工具调用、函数导入、子进程等方式调用。
---

# 文生图生成技能

**本技能是一个图片生成服务，可供其他 skill 和 agent 调用。**

## 服务声明

本技能提供图片生成能力，其他 skill 或 agent 在需要生成图片时，可以直接调用本技能，无需自行实现图片生成逻辑。
